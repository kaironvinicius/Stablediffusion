...
| --- | --- | --- | --- | --- |
| `sd-turbo` | 2 | 0.0 | 768 | Default. The only preset with tolerable CPU latency. |
...
| Run | Time |
| --- | --- |
| `sd-turbo`, 2 steps, 512px | 14.9s |
| `sd-turbo`, 2 steps, 768px | 36.8s |
| `sd-turbo`, 2 steps, 1024px | 111s |

Loading the model on a cold cache adds roughly 9s on top of the first run.
Above 512px the model starts duplicating subjects, since that is the size it
was trained at. 768px is the default as the best balance; 1024px trades a
visibly busier composition for detail.
